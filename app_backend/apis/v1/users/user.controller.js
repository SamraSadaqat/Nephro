  ...
  try {
    ...
    if (account_type == 0) {
      ...
      if (Errors.length != 0) {
        ...
        let avatarImg = helper.fetchS3UploadedFilePath(req.files, "avatar");
        obj = {
          name,
          email,
          mobile,
          ...
          state,
          zipcode,
          avatar: avatarImg && avatarImg.length > 0 ? avatarImg[0] : "",
          password,
          account_type: 0,
        };
      }
...
let getUserProfile = async (req, res) => {
  try {
    let result = await userModel.findOne(
      { _id: req.user._id },
      { access_token: 0, isApproved: 0, password: 0, terms: 0 }
    );
